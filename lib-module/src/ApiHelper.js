import { LocalStorageCredentialStore } from "./LocalStorageCredentialStore.js";
...
export class ApiHelper {
  constructor(authentication, options) {
    if (typeof authentication !== "object") {
      throw new Error(
        "authentication object not provided - must provide an authentication object to instantiate GeotabApi"
      );
    }
    this.options = {
      fullResponse:
        typeof options.fullResponse === "undefined"
          ? false
          : options.fullResponse,
      rememberMe:
        typeof options.rememberMe === "undefined" ? true : options.rememberMe,
      timeout: typeof options.timeout === "undefined" ? 180 : options.timeout,
      newCredentialStore:
        typeof options.newCredentialStore === "undefined"
          ? false
          : options.newCredentialStore,
    };
    ...
    if (authentication.credentials.sessionId) {
      ...
      if (typeof authentication.path === "undefined") {
        throw new Error(
          "Path not provided - Must provide server path if providing sessionId in authentication object"
        );
      }
    ...
    call = call
      ...
      .then((response) => {
        if (response.data.error) {
          throw new Error(
            `${response.data.error.name} - ${response.data.error.message}`
          );
        }
        return response;
      })
  ...
  async sendAxiosCall(
    ...
    let call = new AxiosCall(
      method,
      this.server,
      params,
      callbackSuccess,
      callbackError
    ).send(this.options.timeout);
    ...
    if (method !== "Authenticate") {
      ...
      call.then((response) => {
        if (response.data.result) {
          let server =
            response.data.result.path === "ThisServer"
              ? this.server
