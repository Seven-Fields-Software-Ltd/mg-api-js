  ...
  async call(method, params, callbackSuccess, callbackError) {
    ...
    if (this._helper.sessionId) {
      ...
      if (storedCredentials) {
        ...
        let auth = await this._helper.getAuthentication(
          this.authenticate,
          callbackError,
          this._helper.rememberMe
        );
    ...
    if (!newSession && this._helper.rememberMe) {
      ...
      if (cred) {
        response = this._helper.fullResponse
          ? {
              data: {
                result: { credentials: cred, path: this._helper.server },
              },
            }
          : { credentials: cred, path: this._helper.server };
        if (callbackSuccess) {
      ...
      if (callbackSuccess) {
        cred = await cred.then((response) =>
          this._helper.fullResponse
            ? response.data.result.credentials
            : response
        );
        response = this._helper.fullResponse
          ? {
          ...
          : { credentials: cred.credentials, path: this._helper.server };
        if (callbackSuccess.length === 1) {
          callbackSuccess(response);
        } else if (callbackSuccess.length === 2) {
